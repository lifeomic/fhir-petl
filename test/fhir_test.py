...
def test_to_range():
    ...
    assert range_dict["low"] == {
        "value": 2,
        "unit": "h",
        "system": "http://unitsofmeasure.org",
        "code": "h",
    }
    assert range_dict["high"] == {
        "value": 4,
        "unit": "h",
        "system": "http://unitsofmeasure.org",
        "code": "h",
...
def test_to_dosage():
    ...
    data = [
        1,
        "i po qd cf",
        ("http://snomed.info/sct", "311504000", "With or after food"),
        "Once per day with food",
        ("18:00:00", None),
        "false",
        ("http://snomed.info/sct", "26643006", "Oral Route (qualifier value)"),
        ("http://snomed.info/sct", "421521009", "Swallow"),
        ("http://terminology.hl7.org/CodeSystem/dose-rate-type", "ordered", "Ordered"),
        (100, "mg", "http://unitsofmeasure.org", "mg"),
        (24, "h", "http://unitsofmeasure.org", "h"),
        (5, "g", "http://unitsofmeasure.org", "g"),
    ]
    ...
    assert dosage["additionalInstruction"] == {
        "coding": [
            {
    ...
    assert dosage["doseAndRate"][0]["type"] == {
        "coding": [
            {
    ...
    assert dosage["doseAndRate"][0]["rateQuantity"] == {
        "value": 24,
        "unit": "h",
        "system": "http://unitsofmeasure.org",
        "code": "h",
    }
    assert dosage["maxDosePerLifetime"] == {
        "value": 5,
        "unit": "g",
        "system": "http://unitsofmeasure.org",
        "code": "g",
...
def test_to_med_administration():
    ...
    data = [
        "e1aa3a08-5c36-49cf-96e4-dcca7c1b7a50",
        "completed",
        "071f8ae4-52fd-4f2d-8090-60d1ef3a4452",
        ("http://hl7.org/fhir/sid/ndc", "49884046905", "Ibuprofen Tab 800 MG"),
        date(2020, 5, 5),
        date(2020, 5, 7),
        "Test note",
        "Test dosage text",
    ...
    assert admin["subject"] == {
        "reference": "Patient/071f8ae4-52fd-4f2d-8090-60d1ef3a4452"
    }
    assert admin["medicationCodeableConcept"] == {
        "coding": [
            {
